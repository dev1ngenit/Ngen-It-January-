...
class BrandController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($validator->passes()) {
            ...
            if (!empty($brand)) {
                if ($globalFunImg['status'] == 1) {
                    File::delete(public_path($uploadPath . '/') . $brand->image);
                    File::delete(public_path($uploadPath . '/thumb/') . $brand->image);
                    File::delete(public_path($uploadPath . '/requestImg/') . $brand->image);
                }
